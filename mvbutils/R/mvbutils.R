...
return( local.return())
   ...
   Rmd_origs <- dir( vigdir, patt='[.]Rmd[.]orig$')
   if( !length( Rmd_origs)){
return( local.return())
   }
   ...
   if( FALSE){
     ...
     precomp <- c( sprintf( 
         'local({ owd <- setwd( "%s/vignettes");', dir.), 
         'try({', sprintf(
         'knitr::knit( "%s", "%s");', Rmd_origs, sub( '.orig$', '', Rmd_origs)),
         '});',
         'setwd( owd);',
...
return( returnList( file_srcref, defs, roxy=matrix( 0L, 0, 2), roxlines=list()))
  ...
  irstart <- iroxy[ start]
  roxy <- cbind( irstart, irend)[ irend > irstart+2,]
...
structure( function() {
  ...
  # Maybe there is an official workaround (since tools:::.shlib_internal seems to work)
...
function(
  ...
  PARALLEL <- PARALLEL && (n>1) && requireNamespace( 'foreach') &&
...
#    mvb.file.copy( file.path( dir., makes.in.top), file.path( sourcedir, makes.in.top))
  ...
  if( (.Platform$OS.type=='windows') && ('inst' %in% names( copies)) && any( grepl( '^inst/libs/', copies$inst)) ) {
    # ... then make sure both the "inst/libs/i386" and "inst/libs/x64" folders exist in destination
    # ... since if a DLL only exists in one of these, the other won't be auto-created
    # This allows installation for both archis, avoiding problems with install.pkg etc
    # ... even if it shouldn't be allowed ! In the long run, all packages should be made to work on 32 and 64 anyway
    # Just do it now; a bit of a hack
...
return()
  ...
  if( build){
    for( vig in viggies){
      ...
      res <- try( buildVignette( file= file.path( vigpath, vig), 
          dir= docdir, ...))
  ...
  if( !build || length( builts)){
    ...
    vig_ind <- tools:::.build_vignette_index( all_vigs)
    vig_ind_file <- file( file.path( docdir, 'index.html'), 
        open='w')
    try( tools:::.writeVignetteHtmlIndex( pkg, vig_ind_file, vig_ind))
    close( vig_ind_file)
